...
### 新增
- 新增 `power` 命令，支持查询宿舍电费
- 电费输出统一格式：剩余度数 + 电费余额（按 0.51 元/度换算）
- 宿舍编号自动翻译（基于官方对照表）
- 支持手动指定宿舍号查询（`-r` 参数）
- 支持只查询宿舍信息（`-i` 参数）
- 支持全量输出（`-a` 参数）
- 统一 SSO 登录流程（CAS + 服务登录）
...
### 新增
- `login` 成功后自动展示当前账户、流量、时长、余额与 IP
- 新增 `info` 子命令，可在不重新登录的情况下查看在线信息
- CLI 支持 `--lang` 与环境变量 `NEU_IPGW_LANG` 切换中英双语输出

### 修复
- 兼容门户 `rad_user_info` 接口可能返回数值类型的字段，提升解析稳定性

### 更改
- 登录/信息输出复用统一的本地化描述，默认中文，英文可按需切换
